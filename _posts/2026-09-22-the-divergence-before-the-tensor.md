...
Here is one sentence: **`Café crème brûlée in São Paulo, naïve résumé`**. Embed it with `sentence-transformers/all-MiniLM-L6-v2` in Python, then embed it with the model's ONNX export in .NET, and compare the two vectors. In [ElBruno.LocalEmbeddings](https://github.com/elbruno/elbruno.localembeddings) 1.6.1 the cosine is **0.333**. It's 0.333 again in [Semantic Kernel](https://github.com/microsoft/semantic-kernel), but only when the app runs under invariant globalization. And it's 0.333 in `Microsoft.ML.Tokenizers` with its accent-stripping option switched *on*, again under invariant globalization. Same model, same weights, same pooling math, and a third of a match.
...
- **Semantic Kernel is exact under .NET's default globalization** and loses accents under invariant globalization. More on that below, because it's the part that surprised me most.
