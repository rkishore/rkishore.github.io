...
![The infrastructure gauntlet as five rows, each a trap then its fix then the trade it cost. One: OOM on a T4 at batch-2 and 512 tokens, fixed by gradient_checkpointing=True, which discards most activations and recomputes them on the backward pass, trading memory for about 2 times slower steps. Two: two T4s make it fail not faster, because the trainer auto-wraps in DataParallel and crashes, resolved by dropping to a single GPU with CUDA_VISIBLE_DEVICES set to 0. Three: escaping to a P100 breaks bitsandbytes 4-bit because NF4 kernels need compute capability 7.5 or higher and the P100 is 6.0, so back to the T4 — the better-sounding card doesn't work. Four: Colab free disconnects long sessions, losing a run at step 157 of 250, with no clean fix. Five: what finally worked was Kaggle on a single T4, about 11 seconds per step, about 45 minutes, stable. Every fix was a trade.](/images/finetuning/finetuning-infra-gauntlet.svg)

Four of those deserve the story, because each one teaches something the config file can't.

## OOM on a T4 — memory for time

First wall: out-of-memory on a T4 at batch-2, 512-token sequences. Why does it OOM at all? The backward pass needs each layer's *activations* — its intermediate outputs from the forward pass — to compute gradients, so by default the framework **stores every layer's activations**, and for a deep model at 512 tokens that stored-activation pile is exactly what overflows the T4's 16 GB. The one-line fix, `gradient_checkpointing=True`, keeps only a handful of activations and **recomputes the rest on the backward pass** as they're needed — so you never hold the full stack at once, and peak memory drops sharply. It isn't free: you run the forward math twice, so steps get **~2× slower.** That's the first trade of the day, and it's the mechanism [Part 1](/2026/07/28/how-lora-and-qlora-work.html) flagged — *activations still must be stored under LoRA*, so memory still scales with batch and sequence length. The "0.4% of parameters are trainable" headline does nothing for your activation memory.

## Two GPUs made it fail, not faster

The one I'd never have predicted — and it kicked off the GPU odyssey. Hoping to go *faster*, I grabbed **2× T4**. The trainer helpfully auto-wraps the model in `DataParallel` — and promptly crashes with `chunk expects at least a 1-dimensional tensor`. A notebook won't do real multi-GPU training out of the box anyway, so the second card is pure liability. **The second GPU didn't speed anything up. It just gave the job a new way to crash.** My instinct was to escape to a different card entirely — which walked me straight into the next wall.

## The P100 that "should" have worked

The different card was a P100, and it hit `named symbol not found in ops.cu`. The cause is a hardware floor the tutorials rarely mention: **4-bit NF4 kernels need compute capability ≥ 7.5 (Turing).** The T4 is 7.5 and works. The P100 is 6.0 (Pascal) — an older, in some ways *beefier* card — and it simply can't run the kernels. **The "better" card doesn't work; the humble T4 does.** So I gave up on exotic hardware and came back to a **single T4**, hiding the extra GPU with `CUDA_VISIBLE_DEVICES=0`. "Runs on a GPU" is not a property of your model; it's a property of the exact silicon you were handed.
