...
Every fine-tuning tutorial is a screenshot of the one run that worked. The real cost of "accessible" fine-tuning is everything *around* that screenshot — a sequence of failures each of which is obvious in hindsight and invisible in advance. Here is the actual gauntlet, in order, with what each fix cost:
...
The one I'd never have predicted. Given 2× T4, the trainer helpfully auto-wraps the model in `DataParallel` — and promptly crashes with `chunk expects at least a 1-dimensional tensor`. A notebook can't do real multi-GPU training anyway, so the second card is pure liability. The fix is to *hide* it: `CUDA_VISIBLE_DEVICES=0`. **The second GPU didn't speed anything up. It just gave the job a new way to crash.**
