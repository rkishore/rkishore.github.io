...
Where does the expansion come from? The **mechanism** is a BERT **Masked-Language-Model head**: for each input token, it scores a distribution over the *entire* 30,522-word vocabulary, then **max-pools** those distributions across all tokens and applies ReLU + log-saturation to get the final sparse weights. Projecting every token onto the whole vocab is *where expansion comes from, for free* &mdash; the model is literally asking "what other words does this text imply?" One honest caveat: SPLADE is still **lexical**, not full semantic search. It bridges the vocabulary gap *partway* &mdash; which is exactly why you still want a dense retriever alongside it.
